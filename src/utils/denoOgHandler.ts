...
const handleGet = async (url: URL) => {
  ...
  const ogImage = (() => {
    const targetOgImage = document?.querySelector('meta[property^="og:image"]')?.getAttribute("content");
  ...
  const favicon = (() => {
    const targetFavicon = document?.querySelector('link[rel="icon"]')?.getAttribute("href");
